...
To let other accounts such as LND and Electrs access to the cookie, a startupnotify paramters is configured in bitcoin config, so after bitcoin runs for a while (look for the first "UpdateTip" after "init message: Done loading" in bitcoin logs) the permission will look like this:
```
ls -l bitcoinclients/
total 4
